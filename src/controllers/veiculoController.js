import {
  createVeiculo as createVeiculoService,
  getVeiculos as getVeiculosService,
  getVeiculoById as getVeiculoByIdService,
  updateVeiculo as updateVeiculoService,
  deleteVeiculo as deleteVeiculoService,
  getMaintenancesByVeiculo as getMaintenancesByVeiculoService
} from '../services/veiculoService.js';

// Criar Veículo
export const createVeiculo = async (req, res) => {
  try {
    const veiculo = await createVeiculoService(req.body);
    res.status(201).json({
...
export const getVeiculoById = async (req, res) => {
  try {
    const veiculo = await getVeiculoByIdService(req.params.id);
    if (!veiculo) {
...
export const updateVeiculo = async (req, res) => {
  try {
    const veiculo = await updateVeiculoService(req.params.id, req.body);
    if (!veiculo) {
      return res.status(404).json({ message: 'Veículo não encontrado' });
    }
    res.json({
...
export const deleteVeiculo = async (req, res) => {
  try {
    const veiculo = await deleteVeiculoService(req.params.id);
    if (!veiculo) {
      return res.status(404).json({ message: 'Veículo não encontrado' });
    }
...
export const getMaintenancesByVeiculo = async (req, res) => {
  try {
    const maintenances = await getMaintenancesByVeiculoService(req.params.id);
...
// Arquivo: veiculoController.js | Modificado em: 27/07/2026
